        ...
        if not config['end2end'] and config['is_multimodal_model']:
            ...
            if os.path.isfile(v_feat_file_path):
                self.v_feat = torch.from_numpy(np.load(v_feat_file_path, allow_pickle=True)).type(torch.FloatTensor).to(
                    self.device)
                # CRITICAL FIX: Normalize image features [0,23.4] -> proper scale
                normalize_img = config['normalize_image_features'] if 'normalize_image_features' in config else True
                if normalize_img:
                    print(f"Normalizing image features: mean={self.v_feat.mean():.4f}, std={self.v_feat.std():.4f}")
                    self.v_feat = F.normalize(self.v_feat, p=2, dim=1)
                    print(f"After normalization: mean={self.v_feat.mean():.4f}, std={self.v_feat.std():.4f}")
            ...
            if os.path.isfile(t_feat_file_path):
                self.t_feat = torch.from_numpy(np.load(t_feat_file_path, allow_pickle=True)).type(torch.FloatTensor).to(
                    self.device)
                # Text features are already well-normalized [-0.26, 0.25]
                normalize_txt = config['normalize_text_features'] if 'normalize_text_features' in config else False
                if normalize_txt:
                    print(f"Normalizing text features: mean={self.t_feat.mean():.4f}, std={self.t_feat.std():.4f}")
                    self.t_feat = F.normalize(self.t_feat, p=2, dim=1)
                    print(f"After normalization: mean={self.t_feat.mean():.4f}, std={self.t_feat.std():.4f}")
